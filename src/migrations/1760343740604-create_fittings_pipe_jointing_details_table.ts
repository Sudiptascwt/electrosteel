...
export class CreateFittingsPipeJointingDetailsTable1760343740604 implements MigrationInterface {

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.createTable(
            new Table({
                name: 'ductile_iron_fittings_pipes_jointing_details',
                columns: [
        ...
        await queryRunner.createForeignKey(
            'ductile_iron_fittings_pipes_jointing_details',
            new TableForeignKey({
                columnNames: ['fittings_pipes_jointing_id'],
                referencedTableName: 'ductile_iron_fittings_pipes_jointing', // parent table
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        const table = await queryRunner.getTable('ductile_iron_fittings_pipes_jointing_details');
        const foreignKey = table.foreignKeys.find(fk => fk.columnNames.indexOf('fittings_pipes_jointing_id') !== -1);
        if (foreignKey) {
            await queryRunner.dropForeignKey('ductile_iron_fittings_pipes_jointing_details', foreignKey);
        }
        await queryRunner.dropTable('ductile_iron_fittings_pipes_jointing_details');
    }
